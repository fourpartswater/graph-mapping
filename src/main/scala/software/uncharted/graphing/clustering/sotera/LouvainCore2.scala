...
object LouvainCore2 {
  ...
  def createLouvainGraph[VD: ClassTag](graph: Graph[VD,Long]) : Graph[VertexState,Long]= {
    ...
    val nodeWeightAndDegree = (ec: EdgeContext[VD, Long, (Long, Int)]) => {
      ec.sendToSrc((ec.attr, 1))
      ec.sendToDst((ec.attr, 1))
    }
    val nodeWeightAndDegreeReducer = (n1: (Long, Int), n2: (Long, Int)) => (n1._1 + n2._1, n1._2 + n2._2)
    val nodeWeightsAndDegrees = graph.aggregateMessages(nodeWeightAndDegree, nodeWeightAndDegreeReducer, TripletFields.EdgeOnly)

    val louvainGraph = graph.outerJoinVertices(nodeWeightsAndDegrees)((vid, data, weightAndDegreeOption)=>
    {
      val (weight, degree) = weightAndDegreeOption.getOrElse((0L, 0))
  ...
  private def sendMsg (ec: EdgeContext[VertexState, Long, Map[(Long, Long), Long]]) = {
    ec.sendToDst(Map((ec.srcAttr.community, ec.srcAttr.communitySigmaTot) -> ec.attr))
    ec.sendToSrc(Map((ec.dstAttr.community, ec.dstAttr.communitySigmaTot) -> ec.attr))
  }
...
//    val nodeWeightMapFunc = (ec: EdgeContext[VertexState, Long, Long]) => {
//      ec.sendToSrc(ec.attr)
//      ec.sendToDst(ec.attr)
//    }
//    val nodeWeightReduceFunc = (e1:Long,e2:Long) => e1+e2
//    val nodeWeights = compressedGraph.aggregateMessages(nodeWeightMapFunc, nodeWeightReduceFunc,  TripletFields.EdgeOnly)
...
//    }).cache
    ...
    // Calculate the weighted and unweighted degree of each node
    val nodeDegreeFcn = (ec: EdgeContext[VertexState, Long, (Long, Int)]) => {
      ec.sendToSrc((ec.attr, 1))
      ec.sendToDst((ec.attr, 1))
    }
    val nodeDegreeReduceFcn = (v1: (Long, Int), v2: (Long, Int)) => (v1._1 + v2._1, v1._2 + v2._2)
    val nodeDegrees = compressedGraph.aggregateMessages(nodeDegreeFcn, nodeDegreeReduceFcn, TripletFields.EdgeOnly)

    // Fill in the weighted and unweighted degree of each node
    val louvainGraph = compressedGraph.outerJoinVertices(nodeDegrees)((vid, data, degreesOption) => {
      val (weight, degree) = degreesOption.getOrElse((0L, 0))
      data.communitySigmaTot = weight + data.internalWeight
      data.nodeWeight = weight
      data.nodeDegree = degree
      data
