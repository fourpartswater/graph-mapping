...
class ForceDirectedLayout (parameters: ForceDirectedLayoutParameters = ForceDirectedLayoutParameters.default) extends Serializable {
  ...
  def layoutConnectedNodes (nodes: Seq[GraphNode], edges: Iterable[GraphEdge], parentId: Long, bounds: Circle,
    ...
    val terms = new ForceDirecedLayoutTerms(numNodes, bounds.radius, parameters, edges.map(_.weight).max)
  ...
  private def getForces (terms: ForceDirecedLayoutTerms, bounds: Circle, random: Random): Seq[Force] = {
    Seq(
      ...
      if (parameters.gravity > 0.0) {
        Some(new GravitationalForce(parameters.gravity, bounds.center, terms.kInv))
