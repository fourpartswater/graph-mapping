...
class OpenOrdLayoutTerms (numNodes: Int, maxRadius: Double,
  ...
  val initialTemperature: Double = 0.3 * maxRadius //increase a bit more, to get out of initial local minima (?) //0.1 * maxRadius
  ...
  val nodeOverlapRepulsionFactor = 8.0 //try reducing to keep highly connected nodes clumped together! //64.0
